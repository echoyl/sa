...
class DevService
{
    ...
    public function createModelFile($names,$data)
    {
        ...
        [$use_namespace,$tpl] = $this->createModelRelation($data);
    ...
    public function createControllerRelation($model)
    {
        ...
        if($model['columns'])
        {
            ...
            {
                if(!isset($column['form_type']) || in_array($column['form_type'],['textarea','dateTime','tinyEditor']))
                {
    ...
    public function modelColumn2JsonTable($model,$col)
    {
        ...
        if($name && $relation)
        {
            ...
            $key = [$this->uncamelize($relation['name'])];
            $d['dataIndex'] = array_merge($key,explode('.',$name));
        }
